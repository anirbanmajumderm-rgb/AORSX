...
const emptyForm = { title: "", description: "", icon: "Bot", order: 0, isActive: true };
...
export default function AdminServicesPage() {
  ...
  async function toggleActive(item: Service) {
    try {
      const res = await fetch(`/api/services/${item.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ isActive: !item.isActive }) });
      const json = await res.json();
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-white/40 mt-1">Manage your service offerings</p>
        </div>
      ...
      {editingId !== null && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-6 rounded-2xl bg-white/[0.02] border border-white/[0.04] p-5 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold font-heading">{editingId === -1 ? "New Service" : "Edit Service"}</h3>
      ...
      <div className="space-y-3">
        {items.length === 0 && (
          <div className="rounded-2xl bg-white/[0.02] border border-dashed border-white/[0.06] p-12 text-center">
            <p className="text-sm text-white/30">No services yet. Click &ldquo;New Service&rdquo; to create one.</p>
          </div>
        )}
        {items.map((item) => (
            ...
            <div className="p-5">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  {item.description && <p className="text-sm text-white/40 mt-1 line-clamp-1">{item.description}</p>}
      ...
      <ConfirmModal open={!!deleteTarget} onConfirm={handleDelete} onClose={() => setDeleteTarget(null)} title="Delete Service" description={`Are you sure you want to delete "${deleteTarget?.title}"?`} confirmText="Delete" variant="danger" />
    </div>
  );
}
