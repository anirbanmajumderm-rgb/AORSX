...
import { ArrowLeft, Send, User, Bot, ShieldCheck, Trash2, CheckCircle, Mail } from "lucide-react";
  ...
  async function loadConversation() {
    try {
      ...
      if (json.success) {
        ...
        if (json.data.conversation?.unread) {
  ...
  async function handleReply() {
    ...
    try {
      ...
      if (json.success) {
        ...
        setReplyText("");
        setConversation((prev) => prev ? { ...prev, status: "active", unread: false, updatedAt: new Date().toISOString() } : prev);
      ...
      <div className="flex items-start justify-between gap-4 mb-8">
        <div>
          ...
          <div className="flex items-center gap-3 mt-2">
            ...
            {conversation.unread && (
              <span className="text-[10px] text-neon-cyan font-medium">Unread</span>
            )}
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-medium bg-white/[0.04] text-white/30 hover:text-white/50 transition-all"
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-medium bg-white/[0.04] text-red-400/50 hover:text-red-400 hover:bg-red-500/10 transition-all"
          >
            <Trash2 size={12} />
            Delete
          </button>
